%{
	\\\\\ This version creates a mesh for the entire domain, 
	rather than hallowing out the obstacle \\\\\


	NOTE: CAREFUL WITH MATLAB VERSIONS. YOU MIGHT NEED TO CHANGE THE SURFACE IDs
	
	Solves the steady state fluid flow for an in-compressible fluid
...
% >> Run
chi = zeros(mesh.nt,1);
chi(mesh.InsideElements) = -(1-1e-9);

mu = 1+chi;

% Mass matrix
M = RobinMassMatrix2D(mesh,h);	% Using sparse matrix

% "Load" vector
q = RobinLoadVector2D(mesh,gN);

% Stiffness matrix
A = stiffnessMatrix(mesh,mu=mu);	% Using sparse matrix
...
% --- Methods ---
function q = RobinLoadVector2D(mesh,g)
	q = zeros(mesh.nv,1);
	for e = 1:mesh.ne
...
function mat = RobinMassMatrix2D(mesh,h)
...
function mat = stiffnessMatrix(mesh,options)

	arguments
		mesh;
		options.mu = ones(mesh.nt,1);
	end
